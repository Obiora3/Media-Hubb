...
function usePersisted(key,init){
  const [val,setVal]=useState(()=>{try{const s=localStorage.getItem("mh5_"+key);return s?JSON.parse(s):init;}catch{return init;}});
  useEffect(()=>{try{localStorage.setItem("mh5_"+key,JSON.stringify(val));}catch{}},[key,val]);
  return [val,setVal];
}
...
function AIPanel({mpos,receivables,payables,clients,onDraftInvoice,toast,currency}){
  ...
  const callAI=async(prompt,systemPrompt)=>{
    ...
    try{
      ...
      setResult(text);
    }catch(e){setResult("Unable to reach AI. Please check your connection.");}
    setLoading(false);
  };
...
function UsersContent({currentUser}){
  const rc={admin:"badge-purple",manager:"badge-blue",viewer:"badge-gray"};
  return(
    <div>
      <div className="stat-grid" style={{gridTemplateColumns:"repeat(3,1fr)"}}>
        {[{l:"Total",v:USERS.filter(u=>u.role!=="client").length},{l:"Admins",v:USERS.filter(u=>u.role==="admin").length},{l:"Managers",v:USERS.filter(u=>u.role==="manager").length}].map(s=><div key={s.l} className="stat-card"><div className="stat-label">{s.l}</div><div className="stat-value">{s.v}</div></div>)}
      </div>
      <div className="card"><div className="card-header"><span className="card-title">Team Members</span></div>
        {USERS.filter(u=>u.role!=="client").map(u=>(
          <div key={u.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"var(--border)"}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:13,fontWeight:500}}>{u.name}{u.id===currentUser.id&&<span style={{fontSize:10,color:"var(--brand)",marginLeft:4}}>← you</span>}</div><div style={{fontSize:11,color:"var(--text3)"}}>{u.email}</div></div>
...
function LoginScreen({onLogin}){
  ...
  const rl={admin:"Administrator · Full access",manager:"Manager · Create & edit",viewer:"Viewer · Read only",client:"Client Portal · Your campaigns & invoices"};
  return(
    <div className="login-wrap">
      <div className="login-card">
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:32}}><div style={{width:40,height:40,background:"var(--brand)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:16}}>MH</div><span style={{fontSize:20,fontWeight:700}}>MediaHub</span></div>
        <div style={{fontSize:18,fontWeight:600,marginBottom:4}}>Sign in</div>
        <div style={{fontSize:13,color:"#888",marginBottom:24}}>Select your account to continue</div>
        {USERS.map(u=>(
          <button key={u.id} className={`login-user-btn ${sel?.id===u.id?"sel":""}`} onClick={()=>{setSel(u);setErr("");}}>
            <div style={{width:36,height:36,borderRadius:"50%",background:u.color,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:12,flexShrink:0}}>{u.initials}</div>
            <div><div style={{fontSize:13,fontWeight:500}}>{u.name}</div><div style={{fontSize:11,color:"#888"}}>{rl[u.role]}</div></div>
            {sel?.id===u.id&&<span style={{marginLeft:"auto",color:u.color,fontSize:16}}>✓</span>}
          </button>
        ))}
        {err&&<div style={{fontSize:12,color:"#A32D2D",marginBottom:12,textAlign:"center"}}>{err}</div>}
        <button className="btn btn-primary" style={{width:"100%",justifyContent:"center",padding:10,marginTop:8}} onClick={()=>{if(!sel){setErr("Select a user");return;}onLogin(sel);}}>Continue →</button>
        <div style={{marginTop:16,fontSize:11,color:"#bbb",textAlign:"center"}}>Demo · Stage 5 · Client Portal: Zenith Bank</div>
      </div>
    </div>
  );
}
...
function usePWA() {
  ...
  useEffect(() => {
    ...
    // Register service worker (inline via blob for single-file use)
...
function App(){
  const [currentUser,setCurrentUser]=usePersisted("current_user",null);
  const [workspace,setWorkspace]=usePersisted("workspace",WORKSPACES[0]);
  const [mpos,setMpos]=usePersisted("mpos",SEED_MPOS);
  const [clients,setClients]=usePersisted("clients",SEED_CLIENTS);
  const [receivables,setReceivables]=usePersisted("receivables",SEED_REC);
  const [payables,setPayables]=usePersisted("payables",SEED_PAY);
  const [auditLog,setAuditLog]=usePersisted("audit_log",SEED_AUDIT);
  const [darkMode,setDarkMode]=usePersisted("dark_mode",false);
  const [settings,setSettings]=usePersisted("settings",DEFAULT_SETTINGS);
  const [comments,setComments]=usePersisted("comments",{});
  const [budgets,setBudgets]=usePersisted("budgets",SEED_BUDGETS);
  ...
  const [notifications,setNotifications]=usePersisted("notifications",[]);
  ...
  // Apply custom brand colour from settings
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    // Merge: keep existing read state, add new ones
  ...
  if(!currentUser) return <LoginScreen onLogin={u=>{setCurrentUser(u);toast(`Welcome, ${u.name.split(" ")[0]}!`,"info");}}/>;
  if(currentUser.role==="client") return <ClientPortal user={currentUser} receivables={receivables} mpos={mpos} onLogout={()=>setCurrentUser(null)}/>;
  ...
  const reset=()=>{if(!confirm("Reset all data?"))return;["mpos","clients","receivables","payables","audit_log","current_user","reminders_sent","dark_mode","settings","notifications","comments","workspace","budgets"].forEach(k=>localStorage.removeItem("mh5_"+k));window.location.reload();};
      ...
      <aside className={`sidebar ${sOpen?"open":""}`} aria-label="Navigation">
        ...
        <div className="sidebar-footer">
          ...
          <div style={{display:"flex",gap:6}}>
            ...
            {currentUser.role==="admin"&&<button className="btn btn-ghost btn-sm" style={{fontSize:11,color:"var(--text3)"}} onClick={reset}>↺</button>}
          </div>
          <div style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:"var(--text3)",marginTop:8}}><span style={{width:6,height:6,borderRadius:"50%",background:"#3B6D11",display:"inline-block"}}/>Data saved locally</div>
        </div>
      </aside>
